<?php 
   ...
   $path .= "/timetabledolg/header.php";
   include_once($path);
   include_once("navbar.php");
   
   $path = $_SERVER['DOCUMENT_ROOT'];
   $path .= "/timetabledolg/class.database.php";
   include_once($path);
   if(@$_SESSION['name']){
		   ...
		   if(!empty($_POST['generate'])){
			   ...
			   $query = $link->query("SELECT * FROM timetabledolg WHERE semester='$semester' AND course='$course_code' AND course_full_name= '$course_full_name' AND user_id='$user_id'");
			   $query->setFetchMode(PDO::FETCH_ASSOC);
			   ...
			   while($result = $query->fetch()){
					...
					if($id){
							   ...
							   $query = $link->prepare("INSERT INTO timetabledolg (user_id,course_full_name,year,semester,course) VALUES(?,?,?,?,?)");
