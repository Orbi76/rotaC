		...
		<?php
			if($_SESSION['user_id']){
				...
				function deletefac($faculty_id, $user_id){
					$db_connection = new dbConnection();
					$link = $db_connection->connect(); 
					$link->query("DELETE FROM  `timetabledolg`.`faculty` WHERE  `faculty`.`faculty_id` ='$faculty_id' AND `faculty`.`user_id`='$user_id'");
				}
				...
				// This function lists all the timetabledolg created till now.. with options like delete, edit
