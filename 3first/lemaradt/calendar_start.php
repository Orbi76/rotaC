...
for ($i=1; $i<= $day_count; $i++) {
	...
	$sql = 'SELECT * FROM availability WHERE evdate="'.$date.'"';
	$result = $conn->query($sql);
	$row = $result->fetch();
	if ($row>0) {
		$event = "<input name='$date' type='submit' value ='Details' id='$date' onClick='javasript:show_details(this);'>";
	} 

//end get events
	echo '<div class="cal_day">'."space left: ".$row["availability"];
	echo '<div class="day_heading">' . $i . '</div>';
	//show events button
	if($row != 0) { echo "<div class='openings'><br />" .$event . "</div>";}
	//end button
	echo '</div>';
}
